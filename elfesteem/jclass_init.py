...
class CPString(CStruct):
    ...
    def set_value(self, v):
        self.parent_head.get_constant_pool_by_index(
            self.value_value).set_str(v)
...
def eat_descriptor(descr):
    ...
    if (descr[0] == 'L'):
        try:
            ...
            raise ValueError("Not a valid descriptor string: " + descr)
        type = descr[1:end]
        descr = descr[end:]
    else:
...
class CAttribute_code(CStruct):
    ...
    _fields = [("name", "u16"),
               ...
               ("exception_table", "CException_table",
                lambda c:c.exception_table_length),
...
class CLineNumberTable(CStruct):
    ...
    _fields = [("name", "u16"),
               ...
               ("line_number_table", "LineNumberTableEntry",
                lambda c:c.line_number_table_length),
               ]
...
class Jclass_description(CStruct):
    ...
    def get_interfaces(self):
        out = [self.parent_head.get_constant_pool_by_index(
            x).name for x in self.interfaces_value]
        return out
